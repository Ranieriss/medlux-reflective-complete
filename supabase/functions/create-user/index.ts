...
const normalizeAtivo = (value: CreateUserPayload["ativo"]): boolean => {
  ...
  if (typeof value === "string")
    return ["1", "true", "sim", "yes", "ativo"].includes(
      value.trim().toLowerCase(),
    );
  return true;
};
...
const validatePayload = (payload: CreateUserPayload) => {
  ...
  if (!password || password.length < 6) {
    throw new HttpError(
      400,
      "invalid_password",
      "Field 'password' is required with at least 6 characters",
    );
  }
  ...
  if (!PERFIS_VALIDOS.has(perfil)) {
    throw new HttpError(
      400,
      "invalid_perfil",
      "Field 'perfil' must be ADMIN or OPERADOR",
      {
...
const ensureCallerIsAdmin = async (
  ...
  if (
    normalizePerfil((data?.perfil as string | undefined) || "") !== PERFIS.ADMIN
  ) {
    throw new HttpError(
      403,
      "forbidden",
  ...
  try {
    ...
    if (!resolvedAuthUserId) {
      ...
      if (typeof getByEmailApi.getUserByEmail === "function") {
        const { data: userByEmail, error: byEmailError } =
          await getByEmailApi.getUserByEmail(validated.email);
        if (byEmailError || !userByEmail?.user?.id) {
          throw new HttpError(
            500,
            "auth_lookup_failed",
            "Auth user exists but could not be resolved by e-mail",
            {
    ...
    if (upsertError) {
      throw new HttpError(
        500,
        "usuarios_upsert_failed",
        "Auth user was created, but failed to upsert public.usuarios",
        {
