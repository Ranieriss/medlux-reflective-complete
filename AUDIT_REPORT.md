...
const { data } = await supabase
  .from('usuarios')
  .select('*')
  .eq('email', email)  // ✅ Parametrizado automaticamente
```
...
class HorizontalStrategy extends TipoEquipamentoStrategy {
  ...
  validar(medicoes) { /* lógica NBR 14723 */ }
}
```
...
export class ValidacaoNBR14723 {
  ...
  verificarAprovacao(media, desvio, cv) {
    ...
    return media >= this.limites.min && 
           ...
           cv <= 10  // Máximo 10% de coeficiente de variação
  }
  ...
  function validarMedicao(equipamento, medicoes) {
    ...
    switch (equipamento.tipo) {
      ...
      case 'tachas':
        ...
        break
      default:
        throw new Error(`Tipo de equipamento não suportado: ${equipamento.tipo}`)
    }
