        ...
        >
          ...
          >
            ...
            <div class="text-caption">
              ...
              (https://&lt;project&gt;.supabase.co) e
              <strong>VITE_SUPABASE_ANON_KEY</strong> (JWT iniciando com
              eyJ...).
            </div>
            <div class="text-caption">
              Se estiver usando <strong>sb_publishable_...</strong> ou
              <strong>service_role</strong>, substitua pela chave anon/public
              correta.
            </div>
            <div class="text-caption">
              No Vercel: Project → Settings → Environment Variables (Production,
              Preview e Development) e faça novo deploy.
            </div>
          </v-alert>
              ...
              <v-expansion-panel-text>
                <div class="text-caption">
                  <div>
                    ...
                    {{ diagnosticoSupabase.hostname }}
                  </div>
                  <div>
                    <strong>URL definida:</strong>
                    {{ diagnosticoSupabase.urlDefinida ? "sim" : "não" }}
                  </div>
                  <div>
                    <strong>Key definida:</strong>
                    {{ diagnosticoSupabase.keyDefinida ? "sim" : "não" }}
                  </div>
                  <div>
                    <strong>Fonte da key:</strong>
                    {{ diagnosticoSupabase.fonteKey || "não definida" }}
                  </div>
                  <div>
                    <strong>Key mascarada:</strong>
                    {{ diagnosticoSupabase.keyMascarada }}
                  </div>
                  <div>
                    <strong>Env OK:</strong>
                    {{ diagnosticoSupabase.envOk ? "sim" : "não" }}
                  </div>
                  <div>
                    <strong>Supabase client:</strong>
                    {{ diagnosticoSupabase.clientOk ? "ok" : "indisponível" }}
                  </div>
                  <div>
                    <strong>Sessão:</strong>
                    {{ diagnosticoSupabase.sessionStatus }}
                  </div>
                  <div>
                    <strong>Perfil carregado:</strong>
                    {{ diagnosticoSupabase.perfilStatus }}
                  </div>
                  <div>
                    <strong>Teste trechos_medicao:</strong>
                    {{ diagnosticoSupabase.trechosStatus }}
                  </div>
                  <div>
                    <strong>Teste leituras_medicao:</strong>
                    {{ diagnosticoSupabase.leiturasStatus }}
                  </div>
                  <v-btn
                    size="small"
                    variant="text"
                    class="mt-2"
                    @click="executarDiagnostico"
                  >
          ...
          >
            ...
            <div class="text-caption">
              message: {{ erroDetalhes.message || "n/a" }}
            </div>
            <div class="text-caption">
            ...
            <v-card class="glass">
              ...
              <v-card-text>
                <p class="text-body-2 mb-4">
                  Digite seu e-mail cadastrado. Enviaremos um link para
                  redefinir sua senha.
                </p>

                <v-form
                  ref="formRecuperacaoRef"
                  @submit.prevent="enviarRecuperacao"
                >
                  <v-text-field
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (resultado.sucesso) {
      ...
      erroDetalhes.value = resultado.detalhes || {
        status: null,
        message: "",
        code: null,
      };
...
const executarDiagnostico = async () => {
  ...
  diagnosticoRuntime.value.session = authId
    ? `ok (${authId})`
    : "sem sessão ativa";
  ...
  const { error: trechosError } = await supabase
...
const enviarRecuperacao = async () => {
  ...
  try {
    ...
    if (resultado.success) {
      ...
      mensagemRecuperacao.value =
        resultado.error ||
        "Erro ao enviar e-mail de recuperação. Verifique se o e-mail está cadastrado.";
