  ...
  <div>
    ...
    >
      ...
      >
        ...
        <v-card-text>
          <v-list>
            ...
            <v-list-item>
              ...
              <v-list-item-subtitle>
                {{
                  formatarData(auditoriaSelecionada.created_at)
                }}
              </v-list-item-subtitle>
            </v-list-item>
            <v-list-item v-if="auditoriaSelecionada.dados_novos">
              ...
              <v-list-item-subtitle>
                <pre class="text-caption">{{
                  JSON.stringify(auditoriaSelecionada.dados_novos, null, 2)
                }}</pre>
              </v-list-item-subtitle>
            </v-list-item>
          </v-list>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn @click="dialogDetalhes = false">
            Fechar
          </v-btn>
        </v-card-actions>
      </v-card>
...
const auditoriaFiltrada = computed(() => {
  ...
  if (filtros.value.usuario)
    resultado = resultado.filter((a) =>
      a.usuario?.nome
        ?.toLowerCase()
        .includes(filtros.value.usuario.toLowerCase()),
    );
  return resultado;
});
