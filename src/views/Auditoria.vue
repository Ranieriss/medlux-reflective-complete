<template>
  <div>
    ...
    <v-dialog v-model="dialogDetalhes" max-width="700px">
      <v-card v-if="auditoriaSelecionada" class="glass">
          ...
          <v-list>
            <v-list-item>
              ...
              <v-list-item-subtitle
                >{{ auditoriaSelecionada.acao }}</v-list-item-subtitle
              >
            </v-list-item>
            <v-list-item>
              <v-list-item-title>Entidade</v-list-item-title>
              <v-list-item-subtitle
                >{{ auditoriaSelecionada.entidade }}</v-list-item-subtitle
              >
            </v-list-item>
            <v-list-item>
              <v-list-item-title>Data/Hora</v-list-item-title>
              <v-list-item-subtitle
                >{{ formatarData(auditoriaSelecionada.created_at) }}</v-list-item-subtitle
              >
            </v-list-item>
            <v-list-item v-if="auditoriaSelecionada.dados_novos">
              <v-list-item-title>Dados</v-list-item-title>
              <v-list-item-subtitle>
                <pre
                  class="text-caption"
                  >{{ JSON.stringify(auditoriaSelecionada.dados_novos, null, 2) }}</pre
                >
              </v-list-item-subtitle>
            </v-list-item>
          </v-list>
...
const visualizarDetalhes = (item) => {
  auditoriaSelecionada.value = item
