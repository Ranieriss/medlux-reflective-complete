                ...
                >
                  <template #item="{ props, item }">
                    <v-list-item v-bind="props">
                      ...
                      <template #title>
                        <strong>{{ item.raw.codigo }}</strong> -
                        {{ item.raw.nome }}
                      </template>
                      <template #subtitle>
                        {{ item.raw.tipo }} |
                        {{ item.raw.localizacao || "Sem localização" }}
                      </template>
                    </v-list-item>
                  </template>
                </v-autocomplete>
        ...
        <v-card-text class="pa-6">
          <v-row>
              ...
              <v-card variant="outlined">
                <v-card-text>
                  ...
                  <div class="text-h6 font-weight-bold">
                    {{ vinculoSelecionado.equipamento_codigo }}
                  </div>
                  <div class="text-body-2">
                    {{ vinculoSelecionado.equipamento_nome }}
                  </div>
                </v-card-text>
              </v-card>
            </v-col>
            ...
            <v-col cols="12">
              <v-card variant="outlined">
                <v-card-text>
                  ...
                  <div class="text-h6 font-weight-bold">
                    {{ vinculoSelecionado.usuario_nome }}
                  </div>
                  <div class="text-body-2">
                    {{ vinculoSelecionado.usuario_email }}
                  </div>
                </v-card-text>
              </v-card>
            </v-col>
            ...
            <v-col cols="12">
              ...
              <div class="text-caption text-secondary">
                Atualizado em:
                {{ formatarDataHora(vinculoSelecionado.updated_at) }}
              </div>
            </v-col>
          </v-row>
        </v-card-text>
...
const carregarVinculos = async () => {
  ...
  try {
    ...
    vinculos.value = data
      ...
      .sort((a, b) =>
        (a.equipamento_codigo || "").localeCompare(
          b.equipamento_codigo || "",
          "pt-BR",
          { sensitivity: "base" },
        ),
      );
    ...
    const mensagem = isRlsError(error)
      ? getMensagemErroRls()
      : `Erro ao carregar vínculos: ${error?.message || "sem detalhes"}`;
    erroCarregamento.value = mensagem;
    diagnosticsStore.pushEvent({
      type: "api_error",
      source: "VinculosLista:carregarVinculos",
      message: mensagem,
      context: { table: "vinculos" },
      error,
    });
...
const editarVinculo = (vinculo) => {
  ...
  vinculoForm.value = {
    ...
    data_fim: vinculo.data_fim
      ? format(parseISO(vinculo.data_fim), "yyyy-MM-dd")
      : null,
...
const salvarVinculo = async () => {
  ...
  try {
    ...
    const vinculoData = {
      ...
      ...(vinculoForm.value.cautela_url !== undefined
        ? { cautela_url: vinculoForm.value.cautela_url || null }
        : {}),
      ...
      ...(vinculoForm.value.cautela_data_entrega !== undefined
        ? {
            cautela_data_entrega:
              vinculoForm.value.cautela_data_entrega || null,
          }
        : {}),
      ...(vinculoForm.value.cautela_tecnico_responsavel !== undefined
        ? {
            cautela_tecnico_responsavel:
              vinculoForm.value.cautela_tecnico_responsavel || null,
          }
        : {}),
      ...(vinculoForm.value.cautela_treinamento_realizado !== undefined
        ? {
            cautela_treinamento_realizado:
              !!vinculoForm.value.cautela_treinamento_realizado,
          }
        : {}),
    ...
    if (modoEdicao.value) {
      ...
      // Insert
      const result = await supabase.from("vinculos").insert([vinculoData]);
      error = result.error;
    }
    ...
    mostrarSnackbar(
      isRlsError(error) ? getMensagemErroRls() : "Erro ao salvar vínculo",
      "error",
    );
  } finally {
...
const finalizarVinculo = async (vinculo) => {
  try {
    ...
    mostrarSnackbar(
      isRlsError(error) ? getMensagemErroRls() : "Erro ao finalizar vínculo",
      "error",
    );
  }
};
...
const excluirVinculo = async () => {
  ...
  try {
    ...
    mostrarSnackbar(
      isRlsError(error) ? getMensagemErroRls() : "Erro ao excluir vínculo",
      "error",
    );
  } finally {
