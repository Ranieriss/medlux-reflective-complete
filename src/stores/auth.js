...
export const useAuthStore = defineStore("auth", () => {
  ...
  const isAdmin = computed(
    () => normalizePerfil(usuario.value?.perfil) === PERFIS.ADMIN,
  );
  const isTecnico = computed(() => false);
  const isOperador = computed(
    () => normalizePerfil(usuario.value?.perfil) === PERFIS.OPERADOR,
  );
  ...
  const carregarPerfilUsuario = async () => {
    ...
    if (!perfil) {
      throw new Error(
        "Usuário autenticado, mas sem cadastro em public.usuarios (perfil). Contate o ADMIN.",
      );
    }
  ...
  const login = async (email, senha) => {
    try {
      ...
      const settledSession = await withTimeout(
        waitForSession({ maxMs: 12000, stepMs: 300 }),
        15000,
        "waitForSession",
      );
      if (!settledSession?.user?.id) {
        return {
          ...
          etapa: "Session",
          mensagem:
            "Sessão não apareceu após login. Recarregue a página e tente novamente.",
          detalhes: {
            status: 401,
            message: "Sessão ausente após login (waitForSession).",
      ...
      if (!u?.data?.user?.id) {
        return {
          ...
          etapa: "Auth",
          mensagem:
            "Não foi possível validar o usuário no Auth. Recarregue e tente novamente.",
          detalhes: {
            status: 401,
            message: "auth.getUser não retornou user.",
            code: "auth_get_user_missing",
          },
        };
      }

      // 4) carrega perfil (DB + RLS)
      const usuarioPerfil = await withTimeout(
      ...
      if (
        perfilNormalizado !== PERFIS.ADMIN &&
        perfilNormalizado !== PERFIS.OPERADOR
      ) {
        return {
          sucesso: false,
          etapa: "Permissões",
      ...
      if (updateError) {
        if (
          (updateError.message || "").toLowerCase().includes("ultimo_acesso")
        ) {
          console.warn(
            "⚠️ Coluna ultimo_acesso ausente no banco. Execute a migration de correção do Supabase/RLS.",
          );
        } else {
          console.warn(
            "⚠️ Não foi possível atualizar último acesso:",
            updateError.message,
          );
        }
      }
      ...
      const etapa = error?.message?.includes("sessão")
        ? "Session"
        : error?.message?.toLowerCase().includes("perfil") ||
            error?.message?.toLowerCase().includes("public.usuarios")
          ? "Perfil"
          : "Auth";
