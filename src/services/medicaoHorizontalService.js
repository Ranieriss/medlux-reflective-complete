  ...
  try {
    ...
    if (error) {
      throw Object.assign(new Error("Falha ao inserir leitura horizontal."), {
        diagnostic: formatSupabaseError(
          error,
          "Erro inesperado ao inserir em leituras_medicao.",
        ),
      });
    }
    ...
    return {
      ...
      diagnostic:
        error?.diagnostic ||
        formatSupabaseError(error, "Erro ao inserir leitura horizontal."),
    };
  }
}
  ...
  try {
    ...
    if (error) {
      throw Object.assign(new Error("Falha ao inserir leituras horizontais."), {
        diagnostic: formatSupabaseError(
          error,
          "Erro inesperado ao inserir em lote em leituras_medicao.",
        ),
      });
    }
    ...
    return {
      ...
      diagnostic:
        error?.diagnostic ||
        formatSupabaseError(error, "Erro ao inserir leituras horizontais."),
    };
  }
}
...
export async function calcularResultadoEstacao(estacaoId) {
  try {
    const { data, error } = await supabase.rpc("calcular_resultado_estacao", {
...
export async function calcularResultadoSegmento(segmentoId) {
  try {
    const { data, error } = await supabase.rpc("calcular_resultado_segmento", {
...
export async function validarCondicoes(trechoId) {
  try {
    const { data, error } = await supabase.rpc("validar_condicoes_medicao", {
