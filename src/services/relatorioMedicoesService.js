...
class RelatorioMedicoesService {
  ...
  dadosEmpresa = {
    razaoSocial:
      "I.C.D. Indústria, Comércio e Distribuição de Materiais para Infraestrutura Viária Ltda.",
    cnpj: "10.954.989/0001-26",
    inscricaoEstadual: "255.893.574",
    endereco:
      "Rua Juliano Lucchi, 118 – Jardim Eldorado - Palhoça - SC - CEP 88.133-540",
  ...
  async buscarMedicoes(filtros = {}) {
    try {
      let query = supabase.from("vw_calibracoes_status").select(`
          *,
  ...
  async gerarRelatorioPorTipo(tipo, filtros = {}, formato = "pdf") {
    try {
      ...
      if (medicoes.length === 0) {
        throw new Error(
          `Nenhuma medição encontrada para equipamentos do tipo ${tipo}`,
        );
      }
  ...
  async gerarRelatorioErros(dataInicio, dataFim) {
    try {
      ...
      const { data: equipamentosSemMedicao, error: errorEquipSem } =
        await supabase
          .from("vw_calibracoes_status")
          .select("*")
  ...
  calcularEstatisticas(medicoes) {
    ...
    const aprovadas = medicoes.filter(
      (m) => m.status_validacao === "APROVADO",
    ).length;
    const reprovadas = medicoes.filter(
      (m) => m.status_validacao === "REPROVADO",
    ).length;
    const emDia = medicoes.filter(
      (m) => m.status_vencimento === "EM_DIA",
    ).length;
    const vencidas = medicoes.filter(
      (m) => m.status_vencimento === "VENCIDA",
    ).length;
  ...
  calcularEstatisticasPorTipo(medicoes) {
    ...
    medicoes.forEach((m) => {
      const categoria =
        m.tipo_pelicula || m.tipo_material || "Não especificado";
      ...
      if (m.status_validacao === "APROVADO")
        porCategoria[categoria].aprovadas++;
      if (m.status_validacao === "REPROVADO")
        porCategoria[categoria].reprovadas++;
    ...
    Object.keys(porCategoria).forEach((cat) => {
      ...
      porCategoria[cat].valorMedio =
        valores.length > 0
          ? (valores.reduce((a, b) => a + b, 0) / valores.length).toFixed(2)
          : 0;
      porCategoria[cat].taxaAprovacao =
        porCategoria[cat].total > 0
          ? (
              (porCategoria[cat].aprovadas / porCategoria[cat].total) *
              100
            ).toFixed(2)
          : 0;
    });
  ...
  organizarHistorico(medicoes) {
    return medicoes.sort(
      (a, b) => new Date(a.data_calibracao) - new Date(b.data_calibracao),
    );
  }
  ...
  async gerarPDFPorTipo(dados) {
    ...
    yPos = this.adicionarEstatisticasPorCategoria(
      doc,
      yPos,
      dados.estatisticas,
    );
  ...
  async gerarExcelGlobal(dados) {
    ...
    const wsResumo = XLSX.utils.json_to_sheet([
      ...
      { Campo: "Vencidas", Valor: dados.estatisticas.vencidas },
      {
        ...
        Valor: dados.estatisticas.taxaAprovacao,
      },
  ...
  async gerarExcelPorTipo(dados) {
    ...
    // Por Categoria
    const categoriaData = Object.keys(dados.estatisticas.porCategoria).map(
      (cat) => ({
        Categoria: cat,
        ...
        "Taxa Aprovação (%)":
          dados.estatisticas.porCategoria[cat].taxaAprovacao,
  ...
  adicionarCabecalho(doc, titulo, subtitulo) {
    ...
    doc.text(this.dadosEmpresa.razaoSocial, pageWidth / 2, 12, {
      align: "center",
    });
    ...
    doc.text(
      `CNPJ: ${this.dadosEmpresa.cnpj} | ${this.dadosEmpresa.site}`,
      pageWidth / 2,
      17,
      { align: "center" },
    );
