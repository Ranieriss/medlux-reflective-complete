...
class LaudoPDFService {
  ...
  async gerarLaudo(dadosMedicao) {
    try {
      ...
      this.adicionarTitulo(
        doc,
        "LAUDO TÉCNICO DE MEDIÇÃO DE RETRORREFLETÂNCIA",
      );
  ...
  adicionarCabecalho(doc) {
    ...
    doc.text(
      `CNPJ: ${this.dadosEmpresa.cnpj}  |  Insc. Est.: ${this.dadosEmpresa.inscricaoEstadual}`,
      25,
      16,
    );
    ...
    // Telefone
    doc.text(
      `Fone: ${this.dadosEmpresa.telefone}  |  Fax: ${this.dadosEmpresa.fax}`,
      25,
      24,
    );
    ...
    const items = [
      {
        ...
        valor: this.formatarData(dados.data_calibracao),
      },
      {
        label: "Próxima Medição:",
        valor: this.formatarData(dados.proxima_calibracao),
      },
  ...
  adicionarTabelaMedicoes(doc, yPos, dados) {
    ...
    const tableData = valores.map((valor, index) => [
      `Medição ${index + 1}`,
      `${valor.toFixed(2)}`,
      dados.valor_minimo_referencia
        ? `${dados.valor_minimo_referencia.toFixed(2)}`
        : "-",
      valor >= (dados.valor_minimo_referencia || 0)
        ? "✓ Conforme"
        : "✗ Não conforme",
    ]);
  ...
  adicionarResultadoValidacao(doc, yPos, dados) {
    const statusCor =
      dados.status_validacao === "APROVADO"
        ? this.cores.aprovado
  ...
  async adicionarAssinatura(doc, dados) {
    ...
    try {
      ...
      doc.setFontSize(7);
      doc.setTextColor(...this.cores.textoClaro);
      doc.text("Verificar certificado", qrX + qrSize / 2, qrY + qrSize + 4, {
        align: "center",
      });
    ...
    doc.setTextColor(...this.cores.texto);
    doc.text(
      dados.tecnico_responsavel || "Técnico Responsável",
      signatureX,
      yPos + 5,
      { align: "center" },
    );
    ...
    doc.setFontSize(8);
    doc.setTextColor(...this.cores.textoClaro);
    doc.text("Técnico em Medição de Retrorrefletância", signatureX, yPos + 10, {
      align: "center",
    });

    // Issue date
    doc.text(
      `Emitido em: ${this.formatarData(new Date())}`,
      signatureX,
      yPos + 15,
      { align: "center" },
    );
