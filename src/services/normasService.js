...
export async function buscarCriterioNormativo(params = {}) {
  const tipoEquipamento = mapTipoEquipamento(
    params.tipo_equipamento,
    params.tipoEquipamento,
  );
  if (!tipoEquipamento) {
    return {
      success: false,
      error: "Tipo de equipamento inválido para consulta normativa.",
    };
  }
  ...
  if (!config) {
    return {
      success: false,
      error: `Tipo de equipamento sem RPC mapeada: ${tipoEquipamento}`,
    };
  }

  const rpcArgs = config.argsBuilder(params);
  const { data, error } = await executeWithAuthRetry(
    `buscarCriterioNormativo:${config.rpc}`,
    async () => supabase.rpc(config.rpc, rpcArgs),
  );
  ...
  if (valorMinimo === null) {
    return {
      success: false,
      error: friendlyMissingCriteria,
      code: "CRITERIO_NAO_CADASTRADO",
    };
  }
