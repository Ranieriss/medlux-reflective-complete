  ...
  try {
    ...
    if (role === PERFIS.OPERADOR && usuarioId) {
      const { data: vinculos, error: vincError } = await executeWithAuthRetry(
        "buscarEquipamentosDoUsuario:vinculos",
        async () =>
          supabase
            .from("vinculos")
      ...
      const ids = [
        ...new Set(
          (vinculos || []).map((item) => item.equipamento_id).filter(Boolean),
        ),
      ];
      if (!ids.length) return [];

      const { data, error } = await executeWithAuthRetry(
        "buscarEquipamentosDoUsuario:equipamentos_vinculados",
        async () =>
          supabase
            .from("equipamentos")
            .select("*")
    ...
    const { data, error } = await executeWithAuthRetry(
      "buscarEquipamentosDoUsuario:todos",
      async () =>
        supabase
          .from("equipamentos")
          .select("*")
  ...
  try {
    ...
    if (error) {
      ...
      if (
        ...
        lowerMessage.includes("more than 1 row")
      ) {
        throw new Error(
          "Foram encontrados vínculos duplicados para este usuário e equipamento. Contate o administrador.",
        );
      }
      throw error;
    }
