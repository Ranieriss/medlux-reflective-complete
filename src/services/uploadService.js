      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          (blob) => {
            ...
            console.log(
              `✅ Imagem comprimida: ${file.size} → ${compressedFile.size} bytes`,
            );
...
export async function uploadFotosMedicao(files, options = {}) {
  ...
  for (const [index, file] of files.entries()) {
    if (!ALLOWED_IMAGE_TYPES.includes(file.type)) {
      throw new Error(
        `Arquivo inválido: ${file.name}. Utilize JPG, PNG ou WEBP.`,
      );
    }
...
export async function prepararArquivo(file, opcoes = {}) {
  const { comprimir = true, maxWidth = 1920, quality = 0.85 } = opcoes;
