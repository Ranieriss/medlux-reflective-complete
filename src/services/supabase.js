...
const buildMissingEnvProxy = () => {
  ...
  return /** @type {TypedSupabaseClient} */ (new Proxy(
    {},
    {
      get() {
        throw error
...
export const supabase = hasSupabaseEnv ? supabaseClient : buildMissingEnvProxy()
...
export async function executeWithAuthRetry(operationName, operation) {
  try {
    ...
    console.warn('[supabase] falha de autenticação, tentando refresh da sessão', {
      operationName,
