 ...
 */
export async function adicionarLeituras(
  dispositivoId,
  face,
  tipoMedicao,
  valores,
  ...
  try {
    ...
    if (error) {
      throw Object.assign(
        new Error("Falha ao inserir leituras de dispositivos."),
        {
          diagnostic: formatSupabaseError(
            error,
            "Erro inesperado ao inserir em leituras_dispositivos.",
          ),
        },
      );
    }
    ...
    return {
      ...
      diagnostic:
        error?.diagnostic ||
        formatSupabaseError(error, "Erro ao inserir leituras de dispositivos."),
    };
  }
}
 ...
 */
export async function calcularMediaRI(
  dispositivoId,
  face = "frontal",
  tipoMedicao = "inicial",
) {
  try {
    const { data, error } = await supabase.rpc("calcular_media_ri", {
...
export async function buscarValorMinimo(categoria, tipo, cor) {
  try {
    const { data, error } = await supabase.rpc(
      "buscar_valor_minimo_dispositivo",
      {
...
export async function validarDimensoes(dispositivoId) {
  try {
    const { data, error } = await supabase.rpc("validar_dimensoes_tachao", {
      p_dispositivo_id: dispositivoId,
    });
...
export async function validarConformidade(dispositivoId) {
  try {
    const { data, error } = await supabase.rpc(
      "validar_conformidade_dispositivo",
      {
        p_dispositivo_id: dispositivoId,
      },
    );
