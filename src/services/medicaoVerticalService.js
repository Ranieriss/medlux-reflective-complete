...
async function ensureAuthenticatedSession() {
  ...
  if (error || !data?.session?.user?.id) {
    throw Object.assign(
      new Error(
        "Sessão inválida ou ausente. Faça login novamente para salvar leituras.",
      ),
      {
        diagnostic: formatSupabaseError(
          error,
          "Sessão não encontrada para a operação de escrita.",
        ),
      },
    );
  }
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        georreferenciamento_automatico:
          dados.georreferenciamento_automatico || false,
  ...
  try {
    ...
    if (error) {
      throw Object.assign(new Error("Falha ao inserir leituras verticais."), {
        diagnostic: formatSupabaseError(
          error,
          "Erro inesperado ao inserir em leituras_vertical.",
        ),
      });
    }
    ...
    return {
      ...
      diagnostic:
        error?.diagnostic ||
        formatSupabaseError(error, "Erro ao inserir leituras verticais."),
    };
  }
}
...
export async function calcularMediaGeometria(geometriaId) {
  try {
    const { data, error } = await supabase.rpc("calcular_media_geometria", {
...
export async function validarConformidadeCor(medicaoCorId) {
  try {
    const { data, error } = await supabase.rpc("validar_conformidade_cor", {
...
export async function validarConformidadePlaca(placaId) {
  try {
    const { data, error } = await supabase.rpc("validar_conformidade_placa", {
      p_placa_id: placaId,
    });
 ...
 */
export async function buscarValorMinimo(
  tipoPelicula,
  cor,
  anguloObs,
  anguloEnt,
  ...
  try {
    const { data, error } = await supabase.rpc("buscar_valor_minimo", {
