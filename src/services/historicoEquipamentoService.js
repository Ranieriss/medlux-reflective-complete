...
export const STATUS_EQUIPAMENTO = [
  ...
  {
    value: "AGUARDANDO_REPARO",
    title: "Aguardando Reparo",
...
export async function buscarEstatisticasEquipamento(equipamentoId) {
  try {
    ...
    const stats = {
      ...
      total_manutencoes: data.filter((e) => e.tipo_evento === "MANUTENCAO")
        .length,
      total_calibracoes: data.filter(
        (e) =>
          e.tipo_evento === "CALIBRACAO" || e.tipo_evento === "RECALIBRACAO",
      ).length,
      ...
      custo_medio:
        data.length > 0
          ? data.reduce((sum, e) => sum + (parseFloat(e.custo) || 0), 0) /
            data.length
          : 0,
    };
...
export function getTipoEventoConfig(tipo) {
  return (
    TIPOS_EVENTO.find((t) => t.value === tipo) ||
    TIPOS_EVENTO[TIPOS_EVENTO.length - 1]
  );
}

export function getStatusConfig(status) {
  return (
    STATUS_EQUIPAMENTO.find((s) => s.value === status) || STATUS_EQUIPAMENTO[0]
  );
}
