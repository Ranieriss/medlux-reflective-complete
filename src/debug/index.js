...
const shouldRunStorageSignedUrlCheck = () => {
  if (typeof import.meta !== "undefined" && import.meta.env) {
    const envFlag =
      import.meta.env.MEDLUX_DEBUG_STORAGE ||
      import.meta.env.VITE_MEDLUX_DEBUG_STORAGE;
      ...
      if (level === "error") {
        ...
        pushWithLimit(
          debugState.errors,
          {
            ...
            message:
              errorArg?.message ||
              String(errorArg || "console.error sem detalhes"),
...
const runSupabaseChecks = async ({ session } = {}) => {
  ...
  if (!hasSupabaseEnv) {
    checks.db.push({
      table: "supabase-env",
      ok: false,
      error: { message: "Variáveis do Supabase não configuradas." },
    });
    checks.storage.push({
      action: "supabase-env",
      ok: false,
  ...
  if (!shouldRunSignedUrlCheck || !signedUploadPath) {
    checks.storage.push({
      ...
      ok: true,
      skipped: true,
      reason: !shouldRunSignedUrlCheck
        ? "MEDLUX_DEBUG_STORAGE disabled"
        : "invalid path",
    ...
    try {
      const { data, error } =
        await bucket.createSignedUploadUrl(signedUploadPath);
      if (error) {
...
const createAppWindowState = ({ router, authStore, diagnosticsStore }) => ({
  ...
  get lastErrors() {
    return [...debugState.errors, ...(diagnosticsStore?.events || [])].slice(
      0,
      MAX_ERROR_ITEMS,
    );
  },
...
export const generateDiagnosticDump = async ({
  authStore,
  diagnosticsStore,
} = {}) => {
  let session = null;
  ...
  return {
    ...
    build: {
      ...
      commitHash:
        ...
        "indisponível",
      buildTime:
        debugState.buildTime || import.meta.env.VITE_BUILD_TIME || null,
    ...
    storage: {
      ...
      error:
        checks.storage.find((item) => item?.storageError)?.storageError || null,
    },
    rlsHints: debugState.rlsHints.slice(0, MAX_ERROR_ITEMS),
    logs: {
      internal: debugState.logs.slice(0, MAX_LOG_ITEMS),
      errors: [...debugState.errors, ...(diagnosticsStore?.events || [])].slice(
        0,
        MAX_ERROR_ITEMS,
      ),
    },
...
export const setupDebugHooks = ({
  ...
  debugState.routes = router?.getRoutes?.()?.map((route) => ({
  ...
  window.__medlux_debug_dump = async () =>
    generateDiagnosticDump({ authStore, diagnosticsStore });

  window.addEventListener("error", (event) => {
    const error =
      event?.error || new Error(String(event?.message || "Erro desconhecido"));
    pushWithLimit(
      debugState.errors,
      {
        timestamp: toIso(),
  ...
  window.addEventListener("unhandledrejection", (event) => {
    const error =
      event?.reason instanceof Error
        ? event.reason
        : new Error(String(event?.reason || "Promise rejection sem detalhe"));
    pushWithLimit(
      debugState.errors,
      {
        timestamp: toIso(),
  ...
  console.info(
    "🛠️ [MEDLUX DEBUG] hooks disponíveis: window.supabase, window.__app__, window.__medlux_debug_dump()",
  );
